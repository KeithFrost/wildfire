...
module Bttrack
  class InfoHash
    ...
    def peers(args)
      {
        ...
        'peers' => args[:compact] ? peers_compact(args[:numwant]) : peers_dictionary(args)
      }
    end
    ...
    # info_hash home dir
    def path
      path = "#{@config[:torrents_dir]}/#{id[0..1]}/#{id[2..3]}/#{id}"
    ...
    def cleanup
      ...
      return if File.exists?('.cleanup') &&
        Time.now - File.stat('.cleanup').mtime < @config[:cleanup_interval]

      FileUtils.touch('.cleanup')

      Dir.glob('[a-z0-9]*').map do |peer_id|
        delete_peer(peer_id) unless
          Time.now - File.stat(peer_id).mtime < @config[:announce_interval]*2
      end
    ...
    def scrape
      ...
      if !File.exists?('.scrape') ||
        Time.now - File.stat('.scrape').mtime > @config[:scrape_interval]
    ...
    def self.scrape
      # Dir.glob("#{@config[:torrents_dir]}/**/.scrape").map do |f|
      Dir.glob("/var/wildfire/torrents/**/.scrape").map do |f|
        Marshal.load(File.open(f).read) rescue nil
      end.compact
    end
  end
end